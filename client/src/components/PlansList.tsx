...
export function PlansList({plans, selectedPlanId, addPlan, selectPlan, deletePlan, updatePlan}: any) {
  ...
  const handleSubmitAddPlan = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    addPlan({ countryName, planContent });
    setShowForm(false);
  };

  const handleUpdatePlan = (e: React.FormEvent<HTMLFormElement>, planId: string) => {
    setShowUpdateForm(false);
    e.preventDefault();
    updatePlan(planId, editingCountryName, editingPlanContent);
    setShowUpdateForm(false);
  };
  ...
  const selectedPlan = plans.find((plan: any) => plan._id === selectedPlanId);
      ...
      <ul className="flex flex-col gap-4 list-none">
        {plans.map((plan: any) => (
          <li key={plan._id} className="flex flex-col gap-2">
            ...
            <button onClick={() => handleSelectPlan(plan._id)}>{selectedPlanId === plan._id ? 'Selected' : 'Select'}</button>
            <button onClick={() => {setShowUpdateForm(true); setEditingCountryName(plan.countryName); setEditingPlanContent(plan.planContent); setEditingPlanId(plan._id)}}>Update</button>
            {showUpdateForm && plan._id === editingPlanId && (
              <form onSubmit={(e) => handleUpdatePlan(e, plan._id)}>
                <div className="flex flex-col gap-2">
                <textarea  className="border-2 border-blue-400 rounded-xl px-6 py-4 text-2xl focus:outline-none focus:ring-4 focus:ring-blue-300"
                placeholder="Enter the country name" rows={3} value={editingCountryName} onChange={(e) => setEditingCountryName(e.target.value)} />
                <textarea className="border-2 border-blue-400 rounded-xl px-6 py-4 text-2xl focus:outline-none
                 focus:ring-4 focus:ring-blue-300 resize-none" placeholder="Enter your plan content here" rows={20}
                 value={editingPlanContent} onChange={(e) => setEditingPlanContent(e.target.value)} />
                <button type="submit" disabled={editingCountryName.length === 0 || editingPlanContent.length === 0}>Update</button>
